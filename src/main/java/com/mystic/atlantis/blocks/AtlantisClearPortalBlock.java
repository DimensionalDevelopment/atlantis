...
public class AtlantisClearPortalBlock extends CustomPortalBlock implements Waterloggable {
    ...
    public AtlantisClearPortalBlock(Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(AXIS, Direction.Axis.X).with(WATERLOGGED, Boolean.TRUE));
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState newState, WorldAccess world, BlockPos pos, BlockPos posFrom) {
        ...
        return !bl && !newState.isOf(this) && !(new CustomAreaHelper(world, pos, axis2, foundations)).wasAlreadyValid() ? Blocks.WATER.getDefaultState() : super.getStateForNeighborUpdate(state, direction, newState, world, pos, posFrom);
    }
    ...
    public Block getPortalBase(BlockView world, BlockPos pos) {
        ...
        if (pos.getY() < 0 || isWater((World) world, pos)) {
            return null;
        }
        return this.getPortalBase(world, pos.down());
    }
