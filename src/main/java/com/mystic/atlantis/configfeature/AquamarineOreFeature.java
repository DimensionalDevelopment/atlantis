...
public class AquamarineOreFeature extends Feature<AtlantisOreFeatureConfig> {
    public AquamarineOreFeature(Codec<AtlantisOreFeatureConfig> Codec) {
        super(Codec);
    }

    public boolean generate(StructureWorldAccess reader, ChunkGenerator generator, Random random, BlockPos pos, AtlantisOreFeatureConfig config) {
        ...
        double d4 = pos.getY() + random.nextInt(100) - 2;
        double d5 = pos.getY() + random.nextInt(100) - 2;
        ...
        for (int l1 = k; l1 <= k + j1; ++l1) {
            for (int i2 = i1; i2 <= i1 + j1; ++i2) {
                if (l <= reader.getTopY(Heightmap.Type.OCEAN_FLOOR_WG, l1, i2)) {
                    return this.func_207803_a(reader, random, config, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
                }
            }
        }
    ...
    protected boolean func_207803_a(WorldAccess world, Random random, AtlantisOreFeatureConfig config, double v, double v1, double v2, double v3, double v4, double v5, int i4, int i5, int i6, int i7, int i8) {
        int i = 0;
