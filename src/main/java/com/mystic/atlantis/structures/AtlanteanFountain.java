...
public class AtlanteanFountain extends StructureFeature<StructurePoolFeatureConfig> {

    public AtlanteanFountain(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, AtlanteanFountain::createPiecesGenerator, PostPlacementProcessor.EMPTY);
    }
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(
                () -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(Reference.MODID, "atlantean_fountain/start_pool")),
                10
        );
